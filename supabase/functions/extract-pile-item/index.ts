...
// Builds a FLUX-optimised prompt from clothing metadata.
// FLUX is text-to-image so the description needs to be specific about
// garment type, colour, and the desired flat-lay product style.
...
// ArrayBuffer → base64 string using Deno-native APIs (no Buffer dependency).
...
serve(async (req) => {
  ...
  try {
    ...
    // sourceImageBase64 accepted for API compatibility but not used —
    // FLUX.1-schnell is text-to-image; generation is driven by the item metadata.
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        // 429 = rate limited, 503 = model loading — exponential backoff then retry
        ...
        // 402 = free-tier credits exhausted — signal caller to use fallback
        ...
        // Successful response is raw PNG bytes
