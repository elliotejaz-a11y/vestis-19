...
3a. LAYERING RULE: A maximum of 1 jumper or hoodie per outfit. Outerwear (puffer, jacket, coat) and a jumper are mutually exclusive — if you include outerwear, do NOT also include a jumper. Choose one outer layer or the other, never both.
3b. OUTERWEAR RULE: Outerwear is optional — only include it if weather (cold, rain, wind), occasion (outdoor event, formal winter setting), or style context genuinely warrants it. If you include outerwear, select EXACTLY ONE item (one jacket, coat, puffer, or similar). You must never include two or more outerwear items in a single outfit.
