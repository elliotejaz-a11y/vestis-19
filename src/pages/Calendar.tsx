...
import { Outfit, PlannedOutfit } from "@/types/wardrobe";
...
export function CalendarPage({ outfits }: Props) {
  ...
  const getPlannedForDate = (date: Date) => {
    return plannedOutfits.filter((p) => p.planned_date === format(date, "yyyy-MM-dd"));
  };
              ...
              >
                ...
                {hasPlanned && (
                  <div className={cn(
                    ...
                    hasWorn ? "bg-accent/70" : "bg-accent"
                  )} />
                )}
              </button>
            );
      ...
      {selectedDate && (
        <div className="px-5 mt-6 space-y-4">
          ...
          {selectedPlanned.length > 0 ? (
                  ...
                  <div key={planned.id}>
                    ...
                    {!planned.worn && (
                      <Button
                        ...
                        onClick={() => handleMarkWorn(planned.id)}
                      >
                        <Check className="w-3.5 h-3.5 mr-1" /> Mark as Worn
                      </Button>
                    )}
                    {planned.worn && (
                      <p className="text-[11px] text-accent font-medium mt-1">✓ Worn</p>
                    )}
                  </div>
                );
              })}
            ...
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground">No outfit planned. Assign one:</p>
              {outfits.length === 0 ? (
