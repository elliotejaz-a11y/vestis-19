                ...
                <div key={wItem.id} className="rounded-xl bg-muted p-2 text-center relative">
                  <button
                    onClick={async (e) => {
                      ...
                      setWishlistItems((prev) => prev.filter((w: any) => w.id !== wItem.id));
                      toast({ title: "Removed from wish list" });
                    }}
                    className="absolute top-1 right-1 w-5 h-5 rounded-full bg-foreground/60 text-background flex items-center justify-center text-[10px] z-10 hover:bg-foreground/80"
                  >
                    <X className="w-3 h-3" />
                  </button>
                  {wItem.image_url ? (
